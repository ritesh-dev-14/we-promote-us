...
const Footer = () => {
  ...
  return (
    <footer className="w-full bg-[#F5F6FA] pt-24 pb-10 px-6 md:px-12 lg:px-24 font-sans antialiased text-[#231F54] select-none overflow-hidden relative border-t border-gray-200">
      
      {/* BRAND-COHESIVE ACCENT AMBIENT BLURS */}
      <div className="absolute top-0 left-1/4 w-[600px] h-[300px] bg-gradient-to-r from-[#74358C]/4 to-[#231F54]/4 rounded-full blur-[140px] pointer-events-none" />
      <div className="absolute bottom-0 right-1/4 w-[400px] h-[200px] bg-white/60 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto flex flex-col relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-y-12 lg:gap-x-12 pb-20">
          ...
          <div className="md:col-span-12 lg:col-span-5 flex flex-col items-start max-w-md">
            ...
            {/* LOGO CONTAINER */}
            <div className="h-12 mb-6 flex items-center group cursor-pointer">
              <img 
                src={logo} 
                alt="We Promote Logo" 
                className="h-full object-contain transition-transform duration-300 group-hover:scale-[1.01]" 
            ...
            <p className="text-gray-500 font-light text-[15px] leading-[1.75] mb-8">
              An AI-powered digital marketing and web engineering agency dedicated to accelerating organic traffic, maximizing conversion velocity, and driving industry-leading ROAS.
            </p>
            
            {/* Social Platform Icons Layer */}
            <div className="flex items-center gap-3.5">
              {socialPlatforms.map((platform) => (
                <motion.a
                  key={platform.key}
                  href="#"
                  aria-label={platform.name}
                  whileHover={{ y: -2, backgroundColor: "rgba(116, 53, 140, 0.05)", borderColor: "#74358C", color: "#74358C" }}
                  whileTap={{ scale: 0.97 }}
                  className="w-10 h-10 bg-white border border-gray-200 rounded-xl flex items-center justify-center transition-all duration-200 text-gray-400"
                >
                  <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                    <path d={platform.path} />
                  </svg>
          ...
          {/* COLUMN 2: QUICK NAV LINKS */}
          <div className="md:col-span-6 lg:col-span-3 flex flex-col items-start lg:pl-12">
            <h4 className="text-[#74358C] font-bold text-[11.5px] uppercase tracking-[0.15em] mb-7 relative">
              Explore Links
              <span className="absolute -bottom-2 left-0 w-5 h-[2px] bg-[#74358C]/40 rounded" />
            </h4>
            <ul className="flex flex-col gap-4">
              {quickLinks.map((link, idx) => (
                <li key={idx}>
                  <a href={link.href} className="text-gray-500 font-medium text-[15px] tracking-wide hover:text-[#231F54] transition-colors duration-200 block relative group py-0.5">
                    {link.label}
                    <span className="absolute left-0 bottom-0 w-0 h-[1.5px] bg-[#74358C] transition-all duration-300 group-hover:w-4" />
                  </a>
                </li>
              ))}
            </ul>
          ...
          <div className="md:col-span-6 lg:col-span-4 flex flex-col items-start">
            ...
            {/* Brand Cohesive Accent Panel CTA */}
            <div className="w-full bg-white border border-gray-200/80 rounded-2xl p-5 flex flex-col items-start hover:border-gray-300 transition-all duration-300 shadow-[0_4px_20px_rgba(35,31,84,0.01)]">
              <span className="text-gray-400 font-bold text-[11px] uppercase tracking-wider mb-1.5">Have an active inquiry?</span>
              <a 
                href="mailto:hello@wepromote.com" 
                className="text-[#231F54] hover:text-[#74358C] font-bold text-[15px] tracking-wide transition-colors duration-200 flex items-center gap-2 group"
              >
                hello@wepromote.com
                <svg className="w-3.5 h-3.5 text-gray-400 group-hover:text-[#74358C] transition-colors transform group-hover:translate-x-0.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M14 5l7 7m0 0l-7 7m7-7H3" />
                </svg>
              </a>
            </div>
        ...
        {/* LOWER SUB-SHELF LEVEL */}
        <div className="pt-8 border-t border-gray-200/60 flex flex-col sm:flex-row items-center justify-between gap-5 text-center sm:text-left">
          <p className="text-gray-400 font-normal text-[13.5px] tracking-wide">
            &copy; {new Date().getFullYear()} <span className="text-[#231F54] font-bold">We Promote</span>. All rights reserved.
          </p>
          <div className="flex items-center gap-2 text-gray-400 font-bold text-[11px] tracking-[0.2em] uppercase select-none">
            <span className="inline-block w-1.5 h-1.5 rounded-full bg-[#74358C]/70" />
            Engineered For Absolute Scale
          </div>
        </div>
